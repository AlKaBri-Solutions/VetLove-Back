...
public interface TratamientoRepositorio extends JpaRepository<Tratamiento, Long>{
    ...
    @Query(value =  "SELECT COUNT (*) \r\n" + //
                    "FROM TRATAMIENTO\r\n" + //
                    "WHERE FECHA_FIN > DATEADD('MONTH', -1, CURRENT_TIMESTAMP())\r\n"
            , nativeQuery = true)
    int findCountTratamientosLastMonth();
    //SELECT COUNT (*) FROM TRATAMIENTO WHERE FECHA_FIN > DATEADD('MONTH', -1, CURRENT_TIMESTAMP())
    ...
    , nativeQuery = true)
    //@SqlResultSetMapping(name = "CustomResultMapping", classes = {TratamientosXEnfermedadLMConsulta.class})
